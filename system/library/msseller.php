  	...
  	public function __construct($registry) {
		...
		if (isset($this->session->data['customer_id'])) {
			...
			if ($seller_query->num_rows) {
				...
				$this->phone = $seller_query->row['phone'];
			}
  		}
	}
	...
	public function createSeller($data) {
		...
		$sql = "INSERT INTO " . DB_PREFIX . "ms_seller
				SET seller_id = " . (int)$data['seller_id'] . ",
					...
					bank = '" . (isset($data['bank']) ? $this->db->escape($data['bank']) : '') . "',
					branch = '" . (isset($data['branch']) ? $this->db->escape($data['branch']) : '') . "',
