...
class ControllerAccountRegisterSeller extends Controller {
	...
	public function jxsavesellerinfo() {
    	...
    	if ((utf8_strlen($data['seller']['telephone']) < 1) || (utf8_strlen($data['seller']['telephone']) > 96 )) {
      		$json['errors']['seller[telephone]'] = $this->language->get('error_email');
    	} else {
			if ($this->model_account_customer->getTotalCustomersByEmail($data['seller']['telephone'])) {
				$json['errors']['seller[telephone]'] = $this->language->get('error_exists');
			}
		}
		...
		if (empty($json['errors'])) {
			...
			$this->model_account_customer->deleteLoginAttempts($data['seller']['telephone']);
			$this->customer->login($data['seller']['telephone'], $data['seller']['password']);
